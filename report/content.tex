...
A second way to create new types in \Lean is to use structures. A structure is defined as "a specification of a collection of data, possibly with constraints that the data is required to satisfy" in \cite{MIL}. For instance, we can define a \lean{Point} structure, made of two coordinates as follows :
\begin{leancode}
...
Another very powerful feature of \Lean are the type classes. This concept allows to associate %endow types with additional "features" \todo{write that well}
...
\label{subsub:Coxeter2} \todo{Petit paragraphe} 
